...
class DualSpaceKDWithCMA_OT_1(VariousDivergence):
    ...
    def compute_ot_hidden(self, distiller, student_outputs, teacher_outputs, attention_mask_student, attention_mask_teacher, log):
        ...
        for b in range(batch_size):
            ...
            ########################
            # compute edit_distance cost matrices (C_s2t and C_t2s) -> CMEC
            ########################
            ...
            ########################
            # compute local contextual matrices -> CMC
            ########################
            
            #function to compute context representations - base on sliding window contextual representations
